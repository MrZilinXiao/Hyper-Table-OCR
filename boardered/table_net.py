...
table_model = table_net((None, None, 3), 2)


def table_line(model, img, size=(512, 512), hprob=0.5, vprob=0.5, row=50, col=30, alph=15):
    sizew, sizeh = size
    ...
    # i = 2
    # while i < 1000:
    #     with Timer('predict table lines#' + str(i)):
    #         pred = model.predict(np.array([np.array(inputBlob) / 255.0]))
    #     i += 1
    pred = pred[0]
